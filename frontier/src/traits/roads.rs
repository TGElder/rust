    ...
    T: WithWorld + UpdateRoads + Send + Sync,
{
    async fn add_roads(&self, edges: &[Edge]) {
        let to_build = self.with_world(|world| edges.iter().filter(|edge| !world.is_road(edge)).copied().collect()).await;
...
{
    async fn remove_roads(&self, edges: &[Edge]) {
        let to_remove = self.with_world(|world| edges.iter().filter(|edge| world.is_road(edge)).copied().collect()).await;
        let result = RoadBuilderResult::new(to_remove, RoadBuildMode::Build);
        self.update_roads(result).await;
    }
}
