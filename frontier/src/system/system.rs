...
use crate::system::{Context, EventForwarderActor, EventForwarderConsumer};
...
pub struct System {
    pub cx: Context,
    pub avatar_artist: Process<AvatarArtistActor<Context>>,
    pub avatar_visibility: Process<AvatarVisibility<Context>>,
    pub basic_avatar_controls: Process<BasicAvatarControls<Context>>,
    pub basic_road_builder: Process<BasicRoadBuilder<Context>>,
    pub builder: Process<BuilderActor<Context>>,
    pub cheats: Process<Cheats<Context>>,
    pub edge_sim: Process<EdgeBuildSimulation<Context, AutoRoadTravelDuration>>,
    pub event_forwarder: Process<EventForwarderActor>,
    pub labels: Process<Labels<Context>>,
    pub object_builder: Process<ObjectBuilder<Context>>,
    pub pathfinding_avatar_controls: Process<PathfindingAvatarControls<Context>>,
    pub position_sim: Process<PositionBuildSimulation<Context>>,
    pub prime_mover: Process<PrimeMover<Context>>,
    pub resource_targets: Process<ResourceTargets<Context>>,
    pub rotate: Process<Rotate<Context>>,
    pub settlement_sims: Vec<Process<SettlementSimulation<Context>>>,
    pub setup_new_world: Process<SetupNewWorld<Context>>,
    pub setup_pathfinders: Process<SetupPathfinders<Context>>,
    pub speed_control: Process<SpeedControl<Context>>,
    pub town_builder: Process<TownBuilderActor<Context>>,
    pub town_house_artist: Process<TownHouseArtist<Context>>,
    pub town_label_artist: Process<TownLabelArtist<Context>>,
    pub visibility: Process<VisibilityActor<Context>>,
    pub voyager: Process<Voyager<Context>>,
    pub world_artist: Process<WorldArtistActor<Context>>,
    pub world_gen: Process<WorldGen<Context>>,
}
...
impl System {
    pub fn new(params: Parameters, engine: &mut IsometricEngine, pool: ThreadPool) -> System {
        ...
        let config = System {
            ...
            world_gen: Process::new(WorldGen::new(cx.clone_with_name("world_gen")), world_gen_rx),
        };
    ...
    pub async fn start(&mut self) {
        self.world_gen.run_passive(&self.cx.pool).await;
        self.setup_new_world.run_passive(&self.cx.pool).await;
        self.setup_pathfinders.run_passive(&self.cx.pool).await;

        self.world_artist.run_passive(&self.cx.pool).await;
        self.position_sim.run_passive(&self.cx.pool).await;
        self.voyager.run_passive(&self.cx.pool).await;
        self.visibility.run_passive(&self.cx.pool).await;
        self.town_house_artist.run_passive(&self.cx.pool).await;
        self.town_label_artist.run_passive(&self.cx.pool).await;
        self.edge_sim.run_passive(&self.cx.pool).await;
        self.resource_targets.run_passive(&self.cx.pool).await;
        self.rotate.run_passive(&self.cx.pool).await;
        self.town_builder.run_passive(&self.cx.pool).await;
        self.speed_control.run_passive(&self.cx.pool).await;
        for settlement_sim in &mut self.settlement_sims {
            settlement_sim.run_active(&self.cx.pool).await;
        }
        self.prime_mover.run_active(&self.cx.pool).await;
        self.pathfinding_avatar_controls
            .run_passive(&self.cx.pool)
            .await;
        self.object_builder.run_passive(&self.cx.pool).await;
        self.labels.run_passive(&self.cx.pool).await;
        self.cheats.run_passive(&self.cx.pool).await;
        self.builder.run_active(&self.cx.pool).await;
        self.basic_road_builder.run_passive(&self.cx.pool).await;
        self.basic_avatar_controls.run_passive(&self.cx.pool).await;
        self.avatar_visibility.run_active(&self.cx.pool).await;
        self.avatar_artist.run_passive(&self.cx.pool).await;
        self.event_forwarder.run_passive(&self.cx.pool).await;
    ...
    pub async fn pause(&mut self) {
        ...
        self.event_forwarder.drain(&self.cx.pool, false).await;
        self.avatar_artist.drain(&self.cx.pool, true).await;
        self.avatar_visibility.drain(&self.cx.pool, true).await;
        self.basic_avatar_controls.drain(&self.cx.pool, true).await;
        self.basic_road_builder.drain(&self.cx.pool, true).await;
        self.builder.drain(&self.cx.pool, true).await;
        self.cheats.drain(&self.cx.pool, true).await;
        self.labels.drain(&self.cx.pool, true).await;
        self.object_builder.drain(&self.cx.pool, true).await;
        self.pathfinding_avatar_controls
            .drain(&self.cx.pool, true)
            .await;
        self.prime_mover.drain(&self.cx.pool, true).await;
        for settlement_sim in &mut self.settlement_sims {
            settlement_sim.drain(&self.cx.pool, true).await;
        }
        self.speed_control.drain(&self.cx.pool, true).await;
        self.town_builder.drain(&self.cx.pool, true).await;
        self.rotate.drain(&self.cx.pool, true).await;
        self.resource_targets.drain(&self.cx.pool, true).await;
        self.edge_sim.drain(&self.cx.pool, true).await;
        self.town_label_artist.drain(&self.cx.pool, true).await;
        self.town_house_artist.drain(&self.cx.pool, true).await;
        self.visibility.drain(&self.cx.pool, true).await;
        self.voyager.drain(&self.cx.pool, true).await;
        self.position_sim.drain(&self.cx.pool, true).await;
        self.world_artist.drain(&self.cx.pool, true).await;

        self.setup_pathfinders.drain(&self.cx.pool, true).await;
        self.setup_new_world.drain(&self.cx.pool, true).await;
        self.world_gen.drain(&self.cx.pool, true).await;
    }
